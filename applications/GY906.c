 ...
 * Change Logs:
 * Date           Author       Notes
 * 2023-11-16     songz       the first version
 */
#include "GY906.h"
...
#define SMBUS_SCK_H()        rt_pin_write(SCK_GPIO, PIN_HIGH)//HAL_GPIO_WritePin(SCK_GPIO_Port,SCK_Pin,GPIO_PIN_SET)
#define SMBUS_SCK_L()        rt_pin_write(SCK_GPIO, PIN_LOW)//HAL_GPIO_WritePin(SCK_GPIO_Port,SCK_Pin,GPIO_PIN_RESET)
#define SMBUS_SDA_H()        rt_pin_write(SDA_GPIO, PIN_HIGH)//HAL_GPIO_WritePin(SDA_GPIO_Port,SDA_Pin,GPIO_PIN_SET)
#define SMBUS_SDA_L()        rt_pin_write(SDA_GPIO, PIN_LOW)//HAL_GPIO_WritePin(SDA_GPIO_Port,SDA_Pin,GPIO_PIN_RESET)
#define SMBUS_SDA_PIN()      rt_pin_read(SDA_GPIO)             //HAL_GPIO_ReadPin(SDA_GPIO_Port,SDA_Pin)

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
void GY906_Init()
{
    rt_pin_mode(SCK_GPIO, PIN_MODE_OUTPUT_OD);
    rt_pin_mode(SDA_GPIO, PIN_MODE_OUTPUT_OD);
}
/*******************************************************************************
...
{
    rt_tick_t delay = (rt_tick_get() / 4000000 * us);
    while (delay--)
    {
        ;
    }
